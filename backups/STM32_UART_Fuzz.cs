    ...
    public class STM32_UART_Fuzz : BasicDoubleWordPeripheral, IUART
    {
        ...
        public STM32_UART_Fuzz(IMachine machine, uint frequency = 8000000) : base(machine)
        {
         ...
         public void ReadFromFuzzer_PY(byte[] data){
                ...
                Console.WriteLine($"^^^^ReadFromFuzzer_PY_uart() in STM32F4_UART_Fuzz.cs Len : {receiveFifo.Count}");
        ...
        //         }
        //    if((datasize>0 && receiveFifo.Count<=0)){
        ...
        [field: Transient]
        public event Action FuzzInputExhaust;
