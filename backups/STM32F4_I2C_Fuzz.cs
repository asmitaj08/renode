    ...
    public sealed class STM32F4_I2C_Fuzz : SimpleContainer<II2CPeripheral>, IDoubleWordPeripheral, IBytePeripheral, IKnownSize
    {
        ...
        public void ReadFromFuzzer_PY(byte[] data){
            if (data != null && data.Length > 0){
               ...
               Console.WriteLine($"^^^^ReadFromFuzzer_PY_i2c() No data in STM32F4_I2C_Fuzz.cs. Providing default");
            }
                // general_fuzz_data.Clear();
                // general_fuzz_data.AddRange(data);
                // Console.WriteLine($"^^^^ReadFromFuzzer_PY_i2c() in STM32F4_I2C_Fuzz.cs Len : {dataToReceive.Count}");

        }

        // public static void ReadFromFuzzer_Internal(byte[] data){ //WHEN WAS SENT FROM MACHINE.CS BUT IT DIDN'T WORK  
        ...
        // public void ReadFromFuzzer_Internal(){
        //         int datasize = 0;
        //         byte[] fuzzDataIn;
        //     unsafe{
        ...
        //             dataToReceive = new Queue<byte>(fuzzDataIn);
        //             datasize_track = datasize;
        //         // }
        //         // foreach(var p in peripheralsToFuzz) 
        //         // {
        //         //     // p.ReadFromFuzzer_Internal(fuzzDataIn);
        //         //     STM32F4_I2C_Fuzz.ReadFromFuzzer_Internal(fuzzDataIn);
        //         // }
        //     }
        
        // }
        
        public void SetHookAfterFuzzInputExhaust_I2C(Action hook)
        {
            FuzzInputExhaust += hook; 
        }
            ...
            case State.AwaitingAddress:
                ...
                // {
                    ...
                    if(willReadOnSelectedSlave)
                    {
                        ...
                        // if(datasize_track<=0){
                        //     ReadFromFuzzer_Internal();
                        // }
                        // else if(dataToReceive.Count<=0){ //dataToReceive populated by fuzz_input
                        //     datasize_track=0;
                        //     // FuzzInputExhaust?.Invoke();
                        // }
        ...
        [field: Transient]
        public event Action FuzzInputExhaust; // to signal fuzzInputData exhaust (not needed for all peripheral)
